...
GENERATION_SYSTEM_PROMPT = (
    ...
    "- source URL, 관련 질문, 법률 자문 고지는 직접 생성하지 않는다."
)
...
def _build_generation_prompt(message: str, documents: list[RetrievedDocument]) -> str:
    ...
    return (
        ...
        f"{evidence}\n\n"
        "[답변 지시]\n"
        "1. 검색된 근거에서 확인되는 내용만 사용한다.\n"
        "2. 첫 1~2문장에 핵심 답을 먼저 쓴다.\n"
        "3. 단순 질문은 고정된 번호 형식을 강제하지 않는다.\n"
        "4. 복합 질문은 필요한 항목만 구분해 답한다.\n"
        "5. 근거에 없는 내용은 확인이 필요하다고 말한다.\n"
        "6. source URL과 법률 자문 고지는 직접 쓰지 않는다."
    )
...
def _build_related_hybrid_prompt(message: str, documents: list[RetrievedDocument]) -> str:
    ...
    return (
        ...
        "2. 일반지식 기반으로 사용자가 확인할 방향을 짧게 안내한다.\n"
        "3. URL을 만들지 않는다.\n"
        "4. 최신 기준은 단정하지 않는다.\n"
        "5. 관련 주제 후보를 답변 본문에 직접 나열하지 않는다."
    )
