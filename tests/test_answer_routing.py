...
@pytest.mark.parametrize(
    ...
    [
        (0.45, AnswerRoute.GROUNDED_RAG.value),
        (0.4501, AnswerRoute.RELATED_HYBRID.value),
        (0.59, AnswerRoute.RELATED_HYBRID.value),
        (0.590001, AnswerRoute.LLM_ONLY.value),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0.60, AnswerRoute.OUT_OF_SCOPE.value, "uncertain_vector_fail"),
    ],
)
...
def test_route_decision_preserves_top_k_distance_and_document_ids(monkeypatch):
    ...
    state = decide_route(
        {
            "message": "전세 계약 질문",
            "documents": [
                _document("law_1", 0.46),
                _document("law_2", 0.55),
                _document("law_3", 0.8),
            ],
            "retrieved_count": 3,
            "domain_guardrail_result": "in_scope",
    ...
    assert state["best_distance"] == 0.46
    assert state["top1_distance"] == 0.46
    assert state["top2_distance"] == 0.55
    ...
    state = decide_route(
        {
            "message": "전세 계약 질문",
            "documents": [
                _document("law_1", 0.46, "전세 보증금을 지키려면 어떻게 해야 하나요?"),
                _document("law_2", 0.55, "전세계약이 끝나면 무엇을 확인하나요?"),
            ],
            "retrieved_count": 2,
            "domain_guardrail_result": "in_scope",
            "domain_category": "real_estate_rental",
...
def test_sources_are_only_exposed_for_grounded_rag(monkeypatch):
    ...
    llm_only = _decide(distance=0.60, guardrail_result="in_scope")
